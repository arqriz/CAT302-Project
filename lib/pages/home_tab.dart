...
class _HomeTabState extends State<HomeTab> {
  ...
  void _showCelebrationDialog(int reward) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            "You've earned +$reward bonus Carbon Points for completing your daily quest. Nature thanks you!"),
        actions: [
          TextButton(
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, userSnapshot) {
        if (userSnapshot.connectionState == ConnectionState.waiting) {
          return const Center(
              child: CircularProgressIndicator(color: mossGreen));
        }
                ...
                Padding(
                  ...
                  child: GridView.count(
                    ...
                    children: [
                      ...
                      _statCard(
                          'Recycled',
                          '${user.totalRecycled.toStringAsFixed(1)}kg',
                          Icons.recycling,
                          Colors.green),
                    ],
                  ),
          ...
          return StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return const Center(
                    child: Text("Waiting for Firestore Index..."));
              }
                  ...
                  gradient: LinearGradient(
                    colors: alreadyClaimed
                        ...
                        : (isDone
                            ...
                            : [mossGreen, const Color(0xFF7A8950)]),
                    ...
                    Row(
                      ...
                      children: [
                        Text(
                            alreadyClaimed
                                ? "Quest Completed!"
                                : (isDone ? "Reward Ready!" : "Daily Quest"),
                    ...
                    if (isDone && !alreadyClaimed)
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () =>
                              _claimDailyReward(userId, rewardAmount),
                          child: Text("CLAIM +$rewardAmount POINTS",
                              style:
  ...
  Widget _buildDigitalTree(int points) {
    IconData treeIcon =
        points < 100 ? Icons.grass : (points < 500 ? Icons.spa : Icons.park);

    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 25),
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
          color: creamWhite, borderRadius: BorderRadius.circular(30)),
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text("Impact Progress",
                    style: TextStyle(color: Colors.grey, fontSize: 12)),
                Text(points < 500 ? "Growing Sprout" : "Mature Stage",
                    style: const TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.bold,
                        color: mossGreen)),
      ...
      child: ListView(
        ...
        children: [
          _buildThemeCard("Eco friendly", Icons.brush_rounded,
              "How to use sustainable materials."),
          _buildThemeCard("Useful items", Icons.shopping_basket_rounded,
              "Minimal packaging guides."),
          _buildThemeCard(
              "Energy Saving", Icons.bolt_rounded, "Electricity saving tips."),
          _buildThemeCard("Water Waste", Icons.yard_rounded,
              "Minimize garden water consumption."),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text("${user.points} Carbon Points",
              style: TextStyle(
        ...
        children: [
          _actionButton(
              ...
              () => Navigator.push(context,
                  MaterialPageRoute(builder: (c) => const LogActivityPage()))),
          _actionButton(
              Icons.menu_book_outlined,
              "Guide",
              () => _showInfoDialog("Recycling Guide",
                  "1. Clean containers\n2. Flatten boxes\n3. Check symbols.")),
          _actionButton(
              Icons.lightbulb_outline,
              "Tips",
              () => _showInfoDialog("Daily Tip",
                  "Recycling one glass bottle saves energy to power a PC for 25 mins!")),
        ],
      ),
    );
  }
  ...
  Widget _actionButton(IconData icon, String label, VoidCallback onTap) {
    return Column(
      children: [
        ...
        Text(label,
            style: const TextStyle(
                fontSize: 12, fontWeight: FontWeight.bold, color: mossGreen)),
      ],
    );
  }
  ...
  Widget _buildSectionHeader(String title) {
    return Padding(
        ...
        child: Text(title,
            style: const TextStyle(
                fontSize: 20, fontWeight: FontWeight.bold, color: mossGreen)));
  }
  ...
  Widget _statCard(String label, String val, IconData icon, Color iconColor) {
    return Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
            color: creamWhite, borderRadius: BorderRadius.circular(24)),
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(icon, color: iconColor, size: 30),
          const SizedBox(height: 8),
          Text(val,
              style: const TextStyle(
                  fontSize: 18, fontWeight: FontWeight.bold, color: mossGreen)),
    ...
    return StreamBuilder<List<Activity>>(
      ...
      builder: (context, snapshot) {
        ...
        return Padding(
          padding: const EdgeInsets.symmetric(horizontal: 25),
          child: Column(
              children: recent.map((a) => _buildActivityItem(a)).toList()),
        );
      },
    );
  }
  ...
  Widget _buildActivityItem(Activity activity) {
    return Container(
      ...
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
          color: creamWhite, borderRadius: BorderRadius.circular(20)),
      child: Row(
        children: [
  ...
  void _showInfoDialog(String title, String content) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(title,
            style:
                const TextStyle(color: mossGreen, fontWeight: FontWeight.bold)),
        ...
        actions: [
          TextButton(
              ...
              child: const Text("Got it!",
                  style:
                      TextStyle(color: mossGreen, fontWeight: FontWeight.bold)))
        ],
      ),
    );
  }
